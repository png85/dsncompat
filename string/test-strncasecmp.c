...
int main(int argc, char** argv) {
  ...
  printf("running tests for string/strncasecmp.c:\n");
  ...
  printf("\tr1 = strncasecmp(foo, bar, 255); = %d\n"
	 "\tr2 = strncasecmp(bar, foo, 255); = %d\n", r1, r2);
  if (r1 != -r2) {
  ...
  printf("-- #2: comparision of two strings with similar content in "
	 "different cases:\n"
	 "\tfoo = '%s', bar = '%s'\n", foo, bar);
  r1 = strncasecmp(foo, bar, 255);
  r2 = strncasecmp(bar, foo, 255);
  printf("\tr1 = strncasecmp(foo, bar); = %d\n"
	 "\tr2 = strncasecmp(bar, foo); = %d\n", r1, r2);
  if (r1 != 0 || r2 != 0) {
